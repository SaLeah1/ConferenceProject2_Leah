...
# part one: in which Mendenbar tries to write code and fails
    ...
    print(f"World Size: {length}")
    print(f"View Size: {viewSize}")
    print(f"Genome Length: {viewSize**2}")
    print(f"Steps per Trial: {trialLength}")
    print(f"Trials per Genome: {trials}")
    print(f"Population Size: {populationSize}")
    print(f"Mutation Rate: {mutation_}")
    print(f"Crossover Rate: {crossover_}")
    print(f"Total Epochs: {runs}")
    print("------------")
...
# part two: in which simorine gives up on the literary references
    ...
    for a in range(runs):
        print(f"starting epoch {a+1} of {runs}...")
            ...
            for c in range(trials):
                ...
                for d in range(trialLength):
                    ...
                    for e in range(length):
                        ...
                        for f in sight:
                            string += str(f)
                        index = np.where(stupidArray == string)[0][0]
